...
router.patch('/:id', (req, res) => {
  const { name, color } = req.body;
  const ws = db.prepare('SELECT id FROM workspaces WHERE id = ? AND user_id = ?')
...
router.post('/:id/resources', (req, res) => {
  ...
  res.json(db.prepare('SELECT * FROM workspace_resources WHERE id = ?').get(result.lastInsertRowid));
});
...
router.delete('/:id/resources/:resourceId', (req, res) => {
  ...
  db.prepare('DELETE FROM workspace_resources WHERE id = ? AND workspace_id = ?')
