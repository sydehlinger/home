...
router.get('/', (req, res) => {
  ...
  if (start && end) {
    ...
    ).all(req.session.userId!, start, end) as any[];
  } else {
    meals = db.prepare(
...
router.post('/', (req, res) => {
  ...
  res.json(db.prepare('SELECT * FROM meal_plans WHERE id = ?').get(result.lastInsertRowid));
});

router.patch('/:id', (req, res) => {
  const { name } = req.body;
  const meal = db.prepare('SELECT id FROM meal_plans WHERE id = ? AND user_id = ?')
    .get(req.params.id, req.session.userId!) as any;

  if (!meal) { res.status(404).json({ error: 'Not found' }); return; }

  db.prepare('UPDATE meal_plans SET name = ? WHERE id = ?').run(name, req.params.id);
  res.json(db.prepare('SELECT * FROM meal_plans WHERE id = ?').get(req.params.id));
});
